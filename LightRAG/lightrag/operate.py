...
async def extract_entities(
    ...
    async def _process_single_content(chunk_key_dp: tuple[str, TextChunkSchema]):
        ...
        logger.info(f"[实体提取] Chunk 内容预览 (前200字符): {content[:200]}...")
        ...
        logger.info(f"[实体提取] 开始第一次 LLM 调用 (初始提取): {chunk_key}")
        ...
        logger.info(f"[实体提取] 开始解析提取结果: {chunk_key}")
        ...
        initial_entities_count = len(maybe_nodes)
        ...
        if entity_extract_max_gleaning > 0:
            glean_start_time = time.time()
            logger.info(f"[实体提取] 开始第二次 LLM 调用 (Gleaning): {chunk_key}, max_gleaning={entity_extract_max_gleaning}")
            ...
            glean_parse_start_time = time.time()
            logger.info(f"[实体提取] 开始解析 Gleaning 结果: {chunk_key}")
            ...
            glean_parse_time = time.time() - glean_parse_start_time
        ...
        logger.info(log_message)
        logger.info(f"[实体提取] Chunk {processed_chunks}/{total_chunks} 处理完成，总耗时: {total_time:.2f}秒")
        logger.info(f"[实体提取] 最终统计: 实体={entities_count}, 关系={relations_count}, tokens={chunk_tokens}, content_length={content_length}")
        
        # 如果耗时较长，记录警告
        if total_time > 60:
            logger.warning(f"[实体提取] Chunk {chunk_key} 处理时间较长: {total_time:.2f}秒")
        if pipeline_status is not None:
            async with pipeline_status_lock:
                ...
                if total_chunks > 0:
                    # 提取阶段占30%-80%，共50%的范围
                    # 基础百分比30%，加上提取进度的50%
                    extract_progress = processed_chunks / total_chunks
                    progress_percentage = 30.0 + (extract_progress * 50.0)
                    if "progress" not in pipeline_status:
                        pipeline_status["progress"] = {}
                    pipeline_status["progress"].update({
                        "stage": "extracting",
                        "current": processed_chunks,
                        "total": total_chunks,
                        "percentage": progress_percentage,
                        "stage_details": {
                            **pipeline_status.get("progress", {}).get("stage_details", {}),
                            "extracting": {
                                "current": processed_chunks,
                                "total": total_chunks
                            }
